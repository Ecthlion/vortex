...
fn structural_downgrade_refuses_lower_parts(
    #[case] msp: ArrayRef,
    #[case] lower: ArrayRef,
) -> VortexResult<()> {
    let array =
        DecimalByteParts::try_new_with_lower_parts(msp, vec![lower], DecimalDType::new(38, 2))?;
    assert!(convert::try_to_v1(array.as_view()).is_none());
    let view = convert::to_v2(array.as_view());
    assert!(std::ptr::eq(view.slots(), array.slots()));
    assert!(std::ptr::eq(view.dtype(), array.dtype()));
    Ok(())
}

#[test]
fn legacy_storage_upgrades_without_copying_children() -> VortexResult<()> {
    let child = msp();
    let array = DecimalByteParts::try_new(child.clone(), DecimalDType::new(19, 2))?;
    let legacy = convert::try_to_v1(array.as_view()).vortex_expect("single-child form fits v1");
    assert_eq!(legacy.id(), decimal_byte_parts_v1_id());
    assert!(std::ptr::eq(legacy.slots(), array.slots()));
    assert!(std::ptr::eq(legacy.dtype(), array.dtype()));
    let serialized = v1::serialize(&legacy)?;
    assert_eq!(serialized.metadata, [8, 7]);
    assert!(ArrayRef::ptr_eq(&child, &serialized.children[0]));

    let decoded = v1::deserialize(ArrayDeserialization::new(
        serialized.serialized_id,
        array.dtype(),
        array.len(),
        &serialized.metadata,
    ...
    assert_eq!(
        v1::serialize(&decoded.as_view())?.metadata,
        serialized.metadata
    );
    // The frozen decoder returns storage with no VTable; the explicit upgrade attaches the
    // current VTable and validates its invariants before exposing an executable array.
    let current = convert::from_v1(decoded)?;
    assert_eq!(current.encoding_id(), decimal_byte_parts_v2_id());
    assert!(ArrayRef::ptr_eq(&child, current.msp()));
    Ok(())
}

#[rstest]
#[case::length_mismatch(2, smallvec![Some(msp())])]
#[case::missing_child(3, smallvec![])]
#[case::unsigned_msp(3, smallvec![Some(lower_part())])]
fn attaching_vtable_validates_storage(#[case] len: usize, #[case] slots: vortex_array::ArraySlots) {
    let representation = ArrayRepresentation::new(
        decimal_byte_parts_v1_id(),
        DType::Decimal(DecimalDType::new(19, 2), Nullability::NonNullable),
        len,
        DecimalBytePartsData,
        slots,
    );
    assert!(representation.into_array(DecimalByteParts).is_err());
}
...
fn frozen_decoder_ignores_unknown_metadata_fields() -> VortexResult<()> {
    ...
    let legacy = v1::deserialize(ArrayDeserialization::new(
        decimal_byte_parts_v1_id(),
        &dtype,
        3,
        &metadata,
    ...
    let current = convert::from_v1(legacy)?;
    assert!(ArrayRef::ptr_eq(&child, current.msp()));
    Ok(())
}
