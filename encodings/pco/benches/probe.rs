...
fn probe(bencher: Bencher, array: ArrayRef, indices: &[usize], repeated: bool) {
    bencher
        .with_inputs(|| SESSION.create_execution_ctx())
        .bench_refs(|ctx| {
            let mut retained = repeated.then(|| array.repeated_probe());
            let mut probe = match &mut retained {
                Some(retained) => retained.as_probe(),
                None => array.probe(),
            };
            for &index in indices {
                black_box(
...
fn rle_repeated_first(bencher: Bencher, nullable: bool) {
    probe(bencher, rle(nullable), &indices(1, false), true);
}

#[divan::bench(args = [false, true])]
fn pco_repeated_first(bencher: Bencher, nullable: bool) {
    probe(bencher, pco(nullable), &indices(1, false), true);
}
