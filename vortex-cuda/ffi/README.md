...
`cinclude/vortex_cuda.h` is generated by `build.rs` using cbindgen. Edit the Rust API and
its documentation in `src/lib.rs`, not the generated header. `cbindgen.toml` provides the
standard Arrow Device interface compatibility preamble. Commit regenerated headers alongside
API changes; generation uses stable Rust without macro expansion.
